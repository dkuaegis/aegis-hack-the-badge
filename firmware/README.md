...
`TETRIS STACK`은 10×20 필드와 7종 블록을 사용합니다. `LEFT`/`RIGHT`를
누르는 동안 연속 이동하고 `OK`를 짧게 눌렀다 뗄 때만 회전합니다. `OK`를 길게 눌러도
회전하지 않고 누르는 동안 빠르게 낙하합니다. 10줄마다 레벨과 자동 낙하
속도가 올라가고 `LEFT`+`RIGHT`를 길게 누르면 Home으로 돌아갑니다.
현재 블록 아래의 점 표시는 예상 착지 위치입니다. T 블록 회전 후 중심의
네 모서리 중 3곳 이상이 막힌 상태로 고정되면 T-spin 점수를 적용합니다.
블록 고정 시 짧은 화면 충격과 타격음이 나며, 여러 줄을 동시에 지우면
줄 수에 따라 음이 차례로 상승합니다.

세 게임의 종료 점수는 BLE 브리지로 전송되며, 서버 SQLite DB에 저장된 해당
...
브리지는 공개 개발 키를 기본적으로 거부합니다. 키 오류가 나오거나 인증이
실패하면 최소 32 UTF-8 bytes의 동일한 키로 배지와 브리지를 다시
빌드·실행하세요. 새 상호 인증 프로토콜은 구버전 펌웨어/브리지와 호환되지
않으므로 둘을 함께 업데이트해야 합니다.
